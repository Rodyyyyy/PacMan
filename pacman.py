...
class Ghost:
    ...
    def move(self, player_pos):
        start = (self.x_pos // 30, self.y_pos // 32)
        goal = (player_pos[0] // 30, player_pos[1] // 32)
        path = a_star(start, goal, level)

        if path and len(path) > 1:
            next_position = path[1]  # Get the next position in the path
            self.x_pos = next_position[0] * 30  # Update x position
            self.y_pos = next_position[1] * 32  # Update y position
            self.center_x = self.x_pos + 22
            self.center_y = self.y_pos + 22
